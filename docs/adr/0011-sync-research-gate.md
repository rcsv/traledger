...
- `TripMutation` expresses Cover, user Venue image, external Venue image,
  ...
  progress, Memory, currency, and time-zone intents;
...
This is not the Stage 2 exit. Trip title/date metadata and structural
Day/Activity changes still use `applyPlan`. Same-field resolution and real
