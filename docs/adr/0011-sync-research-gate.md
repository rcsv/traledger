...
- `TripMutation` expresses Cover, user Venue image, external Venue image,
  ...
  append/delete/move/progress, Day replication, mutation-aware reorder Undo,
  Memory, currency, and time-zone intents;
...
This is not the Stage 2 exit. Trip title/date metadata and Day swap still use
`applyPlan`. Same-field resolution, delete-versus-edit policy, concurrent
reorder policy, and real replica convergence also remain unproven.
