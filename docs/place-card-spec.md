...
| macOS regular / narrow | Pass | QA window を 1180×720 / 700×720 で起動し、画像が 176×99 / 88×88 に適応し、画像変更・Maps ボタンが操作可能であることを UI test で確認 |
| macOS keyboard | Pass | Wikimedia attribution を明示的に focusable とし、Tab だけで `hasKeyboardFocus` へ到達する UI test が成功 |
| macOS VoiceOver | Pass | 実際に VoiceOver をオンにした状態で、画像、Venue 情報、Wikimedia 帰属リンク、操作ボタンが Accessibility tree 上の論理順で公開されることを確認。確認後は VoiceOver をオフへ復元 |
...
`TripMap-macOS` scheme では UI test を skip し、62件の deterministic test だけを実行する。
...
Activity reorder / Undo・Redo 1項目、duration suggestion 2項目、Venue search初期状態1項目の
合計13 test が failure 0 で成功した。
最終再実行の結果 bundle は
`DerivedData-VenueImageUITest/Logs/Test/Test-TripMap-VenueImage-QA-2026.07.24_11-04-29-+0900.xcresult`。
...
- Look Around 待機中にキャンセルされた旧 Venue の要求は Wikimedia fallback を開始せず、
  不要になった Venue 名を外部へ送信しない。
